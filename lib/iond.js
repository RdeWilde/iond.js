...
var iondjs = require('bindings')('iondjs.node');
...
Ion.prototype.start = function(options, callback) {
  ...
  iondjs.start(options, function(err, status) {
    ...
    iondjs.onBlocksReady(function(err, result) {
...
Ion.prototype.isSpent = function(txid, outputIndex) {
  return iondjs.isSpent(txid, outputIndex);
};

Ion.prototype.getTransaction = function(txid, queryMempool, callback) {
  return iondjs.getTransaction(txid, queryMempool, callback);
};
...
Ion.prototype.setGenerate = function(options) {
  if (ion.stopping) return [];
  return iondjs.setGenerate(options || {});
};

Ion.prototype.getGenerate = function(options) {
  if (ion.stopping) return [];
  return iondjs.getGenerate(options || {});
};
