...
var paycoindjs = require('../build/Debug/paycoindjs.node');
...
Paycoin.prototype.getTransaction =
Paycoin.prototype.getTx = function(txid, blockhash, callback) {
  if (paycoin.stopping) return [];
  ...
  return paycoindjs.getTransaction(txid, blockhash, function(err, tx) {
    if (err) return callback(err);
    return callback(null, paycoin.tx(tx));
  });
};
